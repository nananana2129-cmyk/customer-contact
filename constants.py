...
MODEL = "gpt-4o-mini"
TEMPERATURE = 0.5
CHUNK_SIZE = 1000  # 500 → 1000に増加（より大きな文脈を保持）
CHUNK_OVERLAP = 200  # 50 → 200に増加（文脈の連続性を向上）
TOP_K = 10  # 5 → 10に増加（より多くの候補を検索）
RETRIEVER_WEIGHTS = [0.3, 0.7]  # [0.5, 0.5] → [0.3, 0.7]に変更（意味検索を重視）
...
DB_NAMES = {
    ...
    DB_INQUIRY_HISTORY_PATH: f"{RAG_TOP_FOLDER_PATH}/inquiry_history"
}
...
SEARCH_INQUIRY_HISTORY_TOOL_DESCRIPTION = "過去の問い合わせ対応履歴を参照したい時に使う。同様の問題の解決方法や過去の回答例を確認できる"
...
SYSTEM_PROMPT_INQUIRY = """
    あなたは当社のカスタマーサポート担当のAIアシスタントです。
    以下の参照情報を基に、ユーザーの質問に丁寧かつ正確に回答してください。

    【重要な指示】
    1. 参照情報の中から質問に関連する情報を**徹底的に**探してください
    2. 人名が含まれる質問の場合、その人物に関する**すべての情報**を抽出してください
    3. 参照情報に該当する情報がある場合は、**具体的に**引用して回答してください
    4. 参照情報に該当する情報がない場合のみ、「具体的な情報が見つかりませんでした」と回答してください
    5. 複数の参照情報がある場合は、それらを統合して包括的な回答を作成してください
    6. 数値や日付などの具体的な情報は正確に引用してください
    7. できる限り詳細に、マークダウン記法を使って回答してください
    8. マークダウン記法で回答する際にhタグの見出しを使う場合、最も大きい見出しをh3としてください

    【参照情報】
    {context}

    【注意事項】
    - 参照情報にない内容は推測や創作をしないでください
    - 不明な点がある場合は、「参照情報からは確認できません」と明記してください
    - 質問の意図を正しく理解し、求められている情報を的確に提供してください
"""
...
SYSTEM_PROMPT_EMPLOYEE_SELECTION = """
    ...
    以下の「出力フォーマット」に従い、従業員情報の中から、問い合わせ内容との関連性が特に高いと思われる
    従業員の「ID」とその「選定理由」を返してください。
    ...
    全ての従業員の従業員IDを返してください。ただし、関連性が低い（対応者候補に含めるべきでない）
    と判断した場合は省いてください。
    - 特に、「過去の問い合わせ対応履歴」と、「対応可能な問い合わせカテゴリ」、また「現在の主要業務」を元に判定を
    行ってください。
    - 一人も対応者候補がいない場合、空文字を返してください。
    - 判定は厳しく行ってください。
    - 選定理由は具体的に、その従業員を選んだ根拠（スキル、経験、過去の対応履歴など）を記述してください。

    # 従業員情報
    {context}

    # 出力フォーマット
    以下の形式でJSON として出力してください:
    {{
        "employee_ids": ["従業員ID1", "従業員ID2", ...],
        "reasons": {{
            "従業員ID1": "○○さんは、△△部で□□を担当しており、過去に同様の××問題に迅速に対応した経験があります。",
            "従業員ID2": "○○さんは、△△部の◇◇として、過去に同様の問い合わせに対応した経験があり、□□に関する専門知識を持っています。",
            ...
        }}
    }}
    
    注意:
    - 理由は「○○さんは、」で始まる自然な文章で記述してください。
    - 従業員の部署、役職、担当業務、過去の経験、専門知識などを具体的に含めてください。
    - 一文で簡潔にまとめてください。
"""
...
SYSTEM_PROMPT_NOTICE_SLACK = """
    ...
    - 【メンション先の選定理由】については、「メンション先の選定理由」をそのまま使用してください。
