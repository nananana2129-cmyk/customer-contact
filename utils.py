...
def run_inquiry_history_chain(param):
    """
    過去の問い合わせ対応履歴参照に特化したTool設定用の関数
    ...
    Returns:
        LLMからの回答
    """
    # 問い合わせ履歴参照に特化したChainを実行してLLMからの回答取得
    ai_msg = st.session_state.inquiry_history_chain.invoke({"input": param, "chat_history": st.session_state.chat_history})
...
def notice_slack(chat_message):
    ...
    # 問い合わせ内容と関連性が高い従業員のID一覧と選定理由を取得
    messages = prompt_template.format_prompt(context=context, query=chat_message).to_messages()
    employee_selection_response = st.session_state.llm(messages)
    ...
    try:
        selection_data = json.loads(employee_selection_response.content)
        employee_ids = selection_data.get("employee_ids", [])
        selection_reasons_dict = selection_data.get("reasons", {})
        logger.info(f"JSON形式で従業員を取得: {employee_ids}")
    except json.JSONDecodeError as e:
        # JSONパースに失敗した場合は従来の方式で処理
        logger.warning(f"JSONパースに失敗、カンマ区切りで処理: {e}")
        try:
            output_parser = CommaSeparatedListOutputParser()
            employee_ids = output_parser.parse(employee_selection_response.content)
            selection_reasons_dict = {}
            logger.info(f"カンマ区切りで従業員を取得: {employee_ids}")
        except Exception as parse_error:
            logger.error(f"従業員ID解析に失敗: {parse_error}, レスポンス: {employee_selection_response.content}")
            employee_ids = []
            selection_reasons_dict = {}
    
    # 従業員が見つからない場合のエラーハンドリング
    if not employee_ids:
        error_msg = "適切な担当者が見つかりませんでした。お手数ですが、直接お問い合わせください。"
        logger.error(f"従業員が見つかりませんでした。問い合わせ内容: {chat_message}")
        return error_msg
    ...
    if not target_employees:
        error_msg = "適切な担当者が見つかりませんでした。お手数ですが、直接お問い合わせください。"
        logger.error(f"従業員情報の照合に失敗。employee_ids: {employee_ids}")
        return error_msg
    ...
    if not slack_ids:
        error_msg = "SlackIDの取得に失敗しました。お手数ですが、直接お問い合わせください。"
        logger.error(f"SlackIDが見つかりませんでした。target_employees: {len(target_employees)}件")
        return error_msg
    ...
    # 選定理由のテキストを生成
    selection_reasons_list = []
    for emp_id in employee_ids:
        reason = selection_reasons_dict.get(emp_id, "選定理由が取得できませんでした")
        selection_reasons_list.append(reason)
    selection_reasons = "\n".join(selection_reasons_list)
    ...
    try:
        ...
        logger.info(f"Slack通知成功: {slack_ids}")
    except Exception as slack_error:
        logger.error(f"Slack通知失敗: {slack_error}")
        return f"担当者への通知中にエラーが発生しました: {str(slack_error)}"
